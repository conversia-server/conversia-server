...
async function startClient(clientId) {
  ...
  // ✅ Listener de mensagens para automação inteligente
...
const WP_API_URL = "https://centrodecriacao.com.br/wp-json/convers-ia/v1/automations";
const activeFlows = {};
...
async function handleIncomingMessage(clientId, message) {
  ...
  if (!conversationState[sender]) {
    ...
    await logInteraction(clientId, sender, text, firstNode.text, firstNode, "", flow.name);
    return;
  }
  ...
  const setor = nextNode.type === "Encaminhar" ? nextNode.text || "atendimento" : "";
  await logInteraction(clientId, sender, text, nextNode.text, nextNode, setor, flow.name);
...
// --- Carrega fluxos periodicamente ---
setInterval(loadAutomations, 60000);
loadAutomations();

// =========================
// 🧾 REGISTRO DE LOGS NO WORDPRESS
// =========================
async function logInteraction(clientId, userNumber, userMessage, botResponse, node, setor, flowName) {
  try {
    await fetch(`${WP_API_URL.replace("/automations", "/logs")}`, {
      method: "POST",
